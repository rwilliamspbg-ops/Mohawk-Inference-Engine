...
class ErrorRecoveryManager:
    ...
    def __init__(self, alert_callback: Callable = None):
        ...
        self.strategies: Dict[str, RecoveryStrategy] = {
            ...
            "WorkerOffline": RecoveryStrategy(
                ...
                parameters={"fallback_mode": "single_worker", "alert_users": True},
            ),
            ...
            "MemoryPressure": RecoveryStrategy(
                ...
                action=RecoveryAction.DEGRADE,
                parameters={"threshold_mb": 80, "action": "reduce_batch_size"},
            ),
            "ModelLoadingError": RecoveryStrategy(
                error_type="ModelLoadingError",
                action=RecoveryAction.ABORT,
                parameters={"rollback_transaction": True},
            ),
        }
        ...
        try:
            ...
            if self.alert_callback:
                await self.alert_callback(
                    "Recovery failed", str(error), str(recovery_error)
                )
            return None

    async def _execute_recovery_strategy(
        self, strategy: RecoveryStrategy, error: Exception, context: Dict[str, Any]
    ) -> Optional[Any]:
    ...
    async def _handle_alert(
        self, error: Exception, context: Dict[str, Any], params: Dict[str, Any]
    ):
    ...
    async def _default_error_handler(self, error: Exception, context: Dict[str, Any]):
        ...
        if self.alert_callback:
            await self.alert_callback(
                "warning", f"Unhandled error: {type(error).__name__}", str(error)
            )
    ...
    async def _execute_with_context(
        self, context: Dict[str, Any], fallback_mode: str = None
    ) -> Any:
    ...
    def get_recovery_stats(self) -> Dict[str, Any]:
        ...
        return {"total_recoveries": total, "by_type": self._recovery_count}
