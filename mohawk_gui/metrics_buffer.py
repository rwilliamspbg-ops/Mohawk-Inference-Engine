...
class BufferedMetrics:
    ...
    def __add__(self, other):
        ...
        return BufferedMetrics(
            ...
            latency_p50=(
                self.latency_p50 * (1 - weight_new) + other.latency_p50 * weight_new
            ),
            latency_p95=(
                self.latency_p95 * (1 - weight_new) + other.latency_p95 * weight_new
            ),
            latency_p99=(
                self.latency_p99 * (1 - weight_new) + other.latency_p99 * weight_new
            ),
...
class MetricsBuffer:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "p95_latency_ms": (
                self._calculate_percentile(latencies, 0.95) if latencies else 0
            ),
            "p99_latency_ms": (
                self._calculate_percentile(latencies, 0.99) if latencies else 0
            ),
            ...
            "buffer_utilization": (
                len(self.buffer) / self.buffer.maxlen if self.buffer.maxlen > 0 else 0
            ),
        }
