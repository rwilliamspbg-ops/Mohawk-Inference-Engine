...
class AuthManager:
    ...
    async def generate_session_token(self, user_id: str, roles: list = None) -> str:
        ...
        payload = {
            ...
            "jti": hashlib.sha256(f"{user_id}{datetime.now()}".encode()).hexdigest()[
                :16
            ],
        }
    ...
    async def refresh_token(self, old_token: str) -> Optional[str]:
        ...
        try:
            ...
            return await self.generate_session_token(
                user_id=verification["user_id"], roles=verification.get("roles", [])
            )
...
class MTLSManager:
    ...
    def generate_certificates(self):
        ...
        print(
            "Certificate generation requires OpenSSL. Use production CA certificates."
        )
