    ...
    try:
        if req.encrypted:
            ...
            if client_id := req.manifest.get("client_id") or "controller":
                ...
                if not aead:
                    raise HTTPException(
                        status_code=400, detail="no handshake for this client"
                    )
                ...
                if not aead:
                    raise HTTPException(
                        status_code=400, detail="no handshake for controller"
                    )
        ...
        slice_obj = WeightSlice.from_bytes(
            blob, start=req.manifest["start"], end=req.manifest["end"]
        )
        ...
        if "pickle" in str(e).lower():
            raise HTTPException(
                status_code=400, detail="Pickle deserialization not supported"
            )
