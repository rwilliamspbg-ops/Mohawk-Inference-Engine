...
async def inference_chat(req: InferenceRequest):
    ...
    try:
        ...
        try:
            ...
            worker_url = os.getenv("MOHAWK_WORKER_URL", "http://mohawk-worker:8003")
...
async def get_metrics():
    ...
    try:
        ...
        with metrics_lock:
            ...
            # Keep GPU simulated or fallback if nvidia-smi doesn't exist
    except Exception:
        pass
    with metrics_lock:
        return dict(metrics)
...
async def connect_workers():
    ...
    try:
        ...
        worker_url = os.getenv("MOHAWK_WORKER_URL", "http://mohawk-worker:8003")
        worker_response = requests.get(f"{worker_url}/health", timeout=5)
        if worker_response.status_code == 200:
...
async def get_discovered_services(service_type: Optional[str] = None):
    ...
    return {"services": [s.to_dict() for s in services], "count": len(services)}
...
async def get_gui_services():
    ...
    return {"guis": [s.to_dict() for s in services], "count": len(services)}
...
async def get_worker_services():
    ...
    return {"workers": [s.to_dict() for s in services], "count": len(services)}
...
async def refresh_service_discovery():
    ...
    return {"status": "refreshing", "message": "Rescanning LAN for Mohawk services..."}
...
def main():
    ...
    parser.add_argument(
        "--discovery", action="store_true", help="Enable LAN service discovery"
    )
    parser.add_argument(
        "--register", action="store_true", help="Register this service for discovery"
    )
