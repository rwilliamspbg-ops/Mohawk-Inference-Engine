...
class VllmInferenceEngine:
    ...
    def tokenized_generate(
        ...
        outputs = self.engine.generate(
            input_ids=input_ids, attention_mask=attention_mask, max_tokens=max_tokens
        )
    ...
    def get_model_profile(self) -> dict:
        ...
        profile = {
            ...
            "cache_hit_rate": (
                self.engine.get_cache_config().cache_hit_rate
                if hasattr(self.engine, "get_cache_config")
                else None
            ),
            "gpu_memory_utilization": self.engine.get_gpu_memory_utilization(),
            "num_tokens_seen": self.engine.get_num_prompt_tokens_seen()
            + self.engine.get_num_generation_tokens_seen(),
        }
