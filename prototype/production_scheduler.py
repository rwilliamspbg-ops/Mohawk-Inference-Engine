...
class ProductionScheduler:
        ...
        for worker in self.workers:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    mem_free_gb = self._parse_metric(metrics_data, "memory_free_gb", 0.0)
                    
                    self.worker_metrics[worker_url] = WorkerHealthMetrics(
                        gpu_utilization=gpu_util,
                        memory_free_gb=mem_free_gb,
                        cpu_utilization=self._parse_metric(metrics_data, "cpu_utilization", 0.0),
                        inference_queue_depth=self._parse_metric(metrics_data, "inference_queue", 0),
                        p50_latency_ms=self._parse_metric(metrics_data, "p50_latency_ms", 0.0),
                        p99_latency_ms=self._parse_metric(metrics_data, "p99_latency_ms", 0.0),
                    )
                ...
                if worker_url not in self.worker_metrics:
                    self.worker_metrics[worker_url] = WorkerHealthMetrics(is_healthy=False, last_error=str(e))
    ...
    def _parse_metric(self, metrics_dict: dict, metric_name: str, default) -> float:
        ...
        count_key = f"{metric_name}_count" if "_count" not in metric_name else metric_name
    ...
    def select_best_worker(
        ...
        candidates = [
            w for w, m in self.worker_metrics.items()
            if m.is_healthy and m.memory_free_gb >= slice_metadata.activation_size_bytes / (1024**3)
        ]
