...
class CostModel:
    ...
    def compute_network_cost(
        ...
        """
        if source_worker.network_bandwidth_gbps <= 0 or target_worker.network_bandwidth_gbps <= 0:
            return float('inf')
        
        bandwidth_bytes_per_sec = (source_worker.network_bandwidth_gbps + 
                                   target_worker.network_bandwidth_gbps) / 2 * 1e9
        
        network_cost = slice_metadata.parameter_count * 4 / bandwidth_bytes_per_sec  # bytes per sec
    ...
    def estimate_latency(
        ...
        if network_latency_ms > 0:
            ...
            network_cost_ms = slice_metadata.parameter_count * 4 / bandwidth_bytes_per_sec * 1000
        
        # Compute latency estimate
        if worker_profile.device_type == 'gpu':
            compute_latency_ms = (slice_metadata.estimated_flops_per_token / 
                                  (worker_profile.gpu_flops_tflops * 1e12)) * 1000
        else:
            compute_latency_ms = (slice_metadata.estimated_flops_per_token / 
                                  (worker_profile.cpu_cores * 30e9)) * 1000
...
class Scheduler:
    ...
    def select_best_worker(
        ...
        for worker in candidates:
            ...
            if (slice_metadata.preferred_device_type == 'gpu' and 
                worker.device_type != 'gpu'):
                continue
            
            # Compute total cost
            compute_cost = self.cost_model.compute_compute_cost(
                slice_metadata, worker
            )
            memory_cost = self.cost_model.compute_memory_cost(
                slice_metadata, worker
            )
    ...
    def get_placement_plan(
        self,
        slices: List[Tuple[int, int, SliceMetadata]]
    ) -> Dict[str, str]:
        """
        ...
        sorted_slices = sorted(
            slices,
            key=lambda s: s[2].activation_size_bytes,
            reverse=True
        )
    ...
    def get_placement_statistics(self) -> Dict[str, Any]:
        ...
        for slice_id, worker_url in self._slice_placements.items():
            placements_per_worker[worker_url] = placements_per_worker.get(worker_url, 0) + 1
        ...
        return {
            ...
            'workers_with_slices': sum(1 for p in placements_per_worker.values() if p > 0),
            'utilization_by_worker': {
                ...
                for w in self.workers if w.is_healthy
            }
        }
...
class TopologyAwareScheduler(Scheduler):
    ...
    def compute_network_cost_with_topology(
        ...
        if (source_worker.url, target_worker.url) in self._network_latencies:
            ...
            network_cost = slice_metadata.parameter_count * 4 / bandwidth_bytes_per_sec * 1000
            ...
            if location1 and location2:
                ...
                import re
                num1 = int(re.search(r'\d+', location1).group()) if re.search(r'\d+', location1) else 0
                num2 = int(re.search(r'\d+', location2).group()) if re.search(r'\d+', location2) else 0
                
                distance_km = abs(num1 - num2) * 10  # Rough estimate: 10km per rack difference
