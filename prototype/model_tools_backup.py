...
class WeightSlice:
    ...
    def from_bytes(cls, data: bytes, start: int, end: int, version: str = "v1.0") -> 'WeightSlice':
        ...
        while i < len(weight_data):
            ...
            try:
                ...
                b = np.frombuffer(b_bytes, dtype=np.float32) if b_bytes else np.array([])
                weights.append((w, b))
            except Exception:
                # Fallback: reshape based on common layer sizes
                w = np.frombuffer(w_bytes, dtype=np.float32).reshape(-1, 8)
                b = np.frombuffer(b_bytes, dtype=np.float32).reshape(-1, 1) if len(b_bytes) > 0 else np.array([])
                weights.append((w, b))
    ...
    def __repr__(self):
        return f"WeightSlice({self.start_layer}:{self.end_layer}, version={self.version})"
