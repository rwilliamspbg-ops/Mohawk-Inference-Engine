...
class MohawkServiceDiscovery:
    ...
    def __init__(self, on_service_added: Optional[Callable] = None,
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.browsers[self.MOHAWK_GUI_TYPE] = ServiceBrowser(
                self.zeroconf, self.MOHAWK_GUI_TYPE, handlers=[self._on_service_state_change]
            )
            
            # Browse for Worker services
            self.browsers[self.MOHAWK_WORKER_TYPE] = ServiceBrowser(
                self.zeroconf, self.MOHAWK_WORKER_TYPE, handlers=[self._on_service_state_change]
            )
            
            logger.info("Service discovery started - listening for Mohawk services on LAN")
            return True
    ...
    def _add_service(self, zeroconf: Zeroconf, service_type: str, name: str):
        ...
        try:
            ...
            addresses = [addr.decode() if isinstance(addr, bytes) else addr
...
class LanServiceRegistry:
    ...
    def __init__(self, hostname: str, service_type: str, port: int,
...
if __name__ == "__main__":
    ...
    discovery = MohawkServiceDiscovery(on_service_added=on_added,
                                       on_service_removed=on_removed)
